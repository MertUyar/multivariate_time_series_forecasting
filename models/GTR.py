...
class GTR(nn.Module):
    def __init__(self, seq_len, pred_len, cycle_len, var_num, period_len=24):
        ...
        self.Q = nn.Parameter(torch.zeros(cycle_len, var_num), requires_grad=True)
    
    def forward(self, x, q, cycle_index): #x -> (B, T, N)
        _, T, N = x.shape
        ...
        out = self.conv2d(stack.reshape(-1, 1, 2, T)).reshape(-1, N, T) # (B, N, T)
        return self.dropout(out).permute(0, 2, 1) # (B, T, N)
